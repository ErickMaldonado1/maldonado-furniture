...
const CategoryCard: React.FC<CategoryCardProps> = ({
    ...
    return (
        <Link
            ...
            className={`group relative overflow-hidden rounded-[2.5rem] bg-zinc-100 dark:bg-zinc-900 aspect-[4/5] shadow-sm hover:shadow-2xl hover:shadow-[#4A3728]/15 transition-all duration-1000 ${className}`}
        >
            <Image
                src={imageSrc}
                alt={label}
            ...
            <div className="absolute inset-x-8 bottom-8 flex flex-col items-start translate-y-4 group-hover:translate-y-0 transition-transform duration-700">
                <span className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-[9px] font-black text-white uppercase tracking-[0.3em] mb-4">
                    {tag}
                </span>
                ...
                <div className="flex items-center gap-3 text-[#4A3728] text-sm font-black opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <span className="text-white uppercase tracking-widest text-[10px]">Ver Colección</span> <HiOutlineArrowNarrowRight size={20} />
                </div>
            </div>
        </Link>
    );
