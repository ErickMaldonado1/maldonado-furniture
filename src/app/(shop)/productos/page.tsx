...
export default async function SearchPage({
  ...
  searchParams: Promise<{ q?: string; discount?: string }>;
}) {
  const { q: query = "", discount } = await searchParams;
  ...
  if (discount === "true") {
    products = products.filter((p) => (p.discount ?? 0) >= 10);
  }
        ...
        {products.length > 0 ? (
          ...
          <div className="space-y-20">
            <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
              ...
              <h2 className="text-2xl md:text-3xl font-black  text-zinc-900 dark:text-white leading">
