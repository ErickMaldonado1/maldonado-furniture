...
const AccordionItem = ({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`text-sm md:text-base font-bold  transition-colors ${isOpen ? "text-[#4A3728]" : "text-zinc-800 dark:text-zinc-200"}`}
        >
          {item.question}
        </span>
    ...
    <main className="min-h-screen bg-[#FDFCFB] dark:bg-[#0A0A0A] md:pt-32 pt-24 pb-24 px-6 md:px-8">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16 space-y-4">
          <span className="text-md font-black uppercase -[0.2em] text-[#4A3728]">
            Soporte al Cliente
          </span>
          <h1 className="text-3xl md:text-4xl font-black uppercase  text-zinc-900 dark:text-white leading-none">
        ...
        <div className="space-y-8">
          {faqData.map((category, catIndex) => (
            <section key={catIndex} className="space-y-4">
              <h2 className="text-[13px] font-black uppercase -[0.2em] text-zinc-400 border-l-4 border-[#4A3728] pl-4 mb-4">
                {category.title}
              </h2>
        ...
        <div className="mt-16 p-8 md:p-12 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-sm text-center space-y-6 shadow-sm">
          ...
          <h3 className="text-xl font-black uppercase  dark:text-white text-zinc-900">
            ¿Aún tienes dudas?
          </h3>
